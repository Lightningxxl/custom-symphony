...
- `project_slug` (string)
  - May be a literal Linear project slugId or `$VAR_NAME`.
  - Canonical environment variable for `tracker.kind == "linear"`: `LINEAR_PROJECT_SLUG_ID`.
...
- `tracker.project_slug`: string or `$VAR`, canonical env `LINEAR_PROJECT_SLUG_ID` when `tracker.kind=linear`
