...
hooks:
  after_create: |
    git clone --depth 1 git@github-persona-backend:Lightningxxl/persona-backend.git .
    test -d .git
  before_remove: |
...
- Context intake hard gate (persona-backend): before implementation, read and apply repo context in this order:
  1. `README.md` (single source of truth; consolidated architecture/protocol/testing/roadmap context)
  2. `PROJECT_LOG.md` (historical decisions and debugging evidence)
  3. `handoff/{{ issue.identifier }}.md` (latest issue-local state)
  4. relevant prior handoff files under `handoff/` when touching shared subsystems.
...
- Primary goal: reliable agent-to-agent relay backend MVP (`request -> accept/reject -> session -> turn relay`), with progressive OpenClaw channel-plugin alignment.
- Read order for context: `README.md` -> `PROJECT_LOG.md` -> relevant `handoff/*.md`.
- Architecture boundaries:
  - `src/server.js` owns HTTP/WS wiring and runtime composition.
  - `src/lib/relay-service.js` owns request/session/turn state transitions and ownership checks.
  - `src/lib/store/*` owns metadata persistence backends.
  - `src/lib/presence/*` owns ephemeral online/heartbeat/active-lock semantics.
  - `src/openclaw/*` owns plugin/framework integration scaffolding and runtime bridge boundaries.
- Storage boundary: persist metadata; do not introduce transcript/payload persistence unless explicitly required by ticket.
- Protocol boundary: if API or WS semantics change, update the protocol section in `README.md` and matching tests in the same change.
- Testing boundary: follow the testing policy documented in `README.md`; test scope must match touched layer (unit/integration/e2e) and `npm test` remains mandatory before push/handoff.
- Runtime boundary: session invariants (`currentTurnIndex`, `currentSpeakerAgentId`, `maxTurns`, termination semantics) must remain coherent.
- OpenClaw boundary: when touching `src/openclaw/*`, keep contract-driven structure (`plugin`, `runtime`) and update `README.md` plus `PROJECT_LOG.md` when semantics shift.
- Current known product blocker from latest repo context: inbound relay connectivity from OpenClaw/plugin back into backend is not fully proven; preserve this fact unless ticket explicitly resolves it with evidence.
...
## Persona Backend Validation Matrix (required)

- Baseline mandatory gate for every code change: `npm test`.
- If touching store/persistence (`src/lib/store/*`, migration-like behavior, metadata shape): run and record
  - `npm run test:unit`
  - `npm run test:integration`
  - `npm test`
- If touching relay runtime/session/request/turn behavior (`src/lib/relay-service.js`, ownership/timeout/termination): run and record
  - `npm run test:unit`
  - `npm run test:integration`
  - `npm run test:e2e`
  - `npm test`
- If touching auth/ws/presence/server routing (`src/server.js`, `src/lib/presence/*`, ws event flow): run and record
  - `npm run test:unit`
  - `npm run test:integration`
  - `npm run test:e2e`
  - `npm test`
- If touching OpenClaw plugin/framework (`src/openclaw/*`, `index.js`, plugin metadata): run and record
  - `npm run test:unit`
  - `npm run test:integration`
  - `npm run test:e2e`
  - `npm test`
...
3.  Immediately reconcile the workpad before new edits:
    ...
    - Confirm context intake has been completed for this run and note any architecture/protocol constraints that apply.
...
5.  Run validation/tests required for the scope.
    ...
    - Apply `Persona Backend Validation Matrix (required)` based on touched modules; record each command and result.
